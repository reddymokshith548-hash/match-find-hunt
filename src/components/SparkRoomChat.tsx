  ...
  const fetchProfile = async () => {
    ...
    try {
      const {
        data,
        error
      } = await supabase.from('profiles').select('id, name') // Also select name just in case, though we only need ID here
      .eq('user_id', user.id).maybeSingle(); // Use maybeSingle to handle 'no row' gracefully
  ...
  const fetchMessages = async () => {
    try {
      const {
        data,
        error
      } = await supabase.from('spark_room_messages').select(`
          id,
          message,
          created_at,
      ...
      try {
        ...
        if (profileIdFromPayload) {
          ...
          const {
            data: profileRes,
            error: profileError
          } = await supabase.from('profiles').select('id, name, profile_pic_url').eq('id', profileIdFromPayload).maybeSingle();
  ...
  const sendMessage = async (e: React.FormEvent) => {
    ...
    try {
      const {
        ...
        room_id: roomId,
        user_id: user.id,
        // Good to keep this, though 'profiles' table uses profile_id
        profile_id: profileId,
        // This is the required foreign key to 'profiles'
      ...
      <CardContent className="flex-1 flex flex-col p-0 overflow-hidden">
        <ScrollArea className="flex-1 p-4">
          {loading ? <div className="flex items-center justify-center h-full">
              ...
              return <div key={msg.id} className={`flex gap-3 ${isOwnMessage ? 'flex-row-reverse' : 'flex-row'}`}>
        ...
        <div className="flex-shrink-0 p-4 border-t">
          <form onSubmit={sendMessage} className="flex gap-2">
            <Input value={newMessage} onChange={e => setNewMessage(e.target.value)} placeholder="Type a message..." disabled={sending || !profileId} // Disable if profileId is missing
            className="flex-1" />
